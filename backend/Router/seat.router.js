...
seatRouter.post("/seatbook",async(req,res)=>{
  ...
  try {
    ...
    const SeatsInSingleRow = await seatModel.aggregate([
    ...
    if (SeatsInSingleRow.length >= numSeats) {
      // Reserve seats in one row
      let temp=[]
      const seatIds = SeatsInSingleRow.map(seat => seat.seats._id);
      await seatModel.updateMany({ _id: { $in: seatIds } }, { $set: { isBooked: true } });
      SeatsInSingleRow.map((seat)=>{
